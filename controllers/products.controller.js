...
exports.updateCategory = (req, res) => {
  ...
  Product.findOne({ static: "static" }).then((doc) => {
    let catObj = doc._categories.find((catObj) => catObj.category == category);
    if (catObj._id != _id && catObj.category == category) {
      res.json({ message: "هذا الصنف موجود بالفعل", error: true });
    } else {
      Product.findOneAndUpdate(
        { static: "static", "_categories._id": _id },
        { "_categories.$.category": category, "_categories.$.order": order }
      ).then(() => {
...
exports.toggleShowOfCategory = (req, res) => {
  ...
  Product.findOneAndUpdate(
    { static: "static", "_categories.category": category },
    { "_categories.$.show": show }
  ).then(() => {
    if (show == true) {
...
exports.updateStatic = (req, res) => {
  ...
  if (body.title) {
    if (body.title == "add") {
      ...
      Product.findOne({ static: "static" }).then((doc) => {
        let catObj = doc._categories.find(
          (catObj) => catObj.category == body.category
        );
        if (catObj) {
          if (file) {
          ...
          let orderFinded = doc._categories.find(
            (catObj) => catObj.order == body.order
          );
          if (orderFinded) {
            if (file) {
              deleteImg(file.filename);
              res.json({ message: "الرقم موجود بالفعل", error: true });
            }
          } else {
            Product.findOneAndUpdate({ static: "static" }, updatedObj).then(
              () => {
                ...
                newAdvertise.save().then(() => {
                  res.json({
                    message: `بنجاح ${body.category} تم أضافة`,
                    error: false,
                  });
                });
              }
      ...
      Product.findOne({ static: "static" }).then((doc) => {
        ...
        if (catObj) {
          ...
          Product.findOneAndUpdate(
            { static: "static" },
            { $pull: { _categories: { category: body.category } } }
          ).then(() => {
...
exports.getProductsByOptions = (req, res) => {
  ...
  if (Object.keys(query).length == 0 || query.category == "all") {
    ...
    Product.find({ $nor: [{ static: "static" }] })
      .sort({ name: 1, addedDate: -1 })
      .then((doc) => {
        res.json(doc);
      });
  } else {
...
exports.addNewProduct = (req, res) => {
  ...
  Product.findOne({ name: body.name })
    .then((doc) => {
      if (doc) {
...
exports.updateProduct = (req, res) => {
  const body = req.body;
  let sizes;
  body.sizes ? (sizes = body.sizes) : (sizes = []);
  let otherImages;
  body.otherImages ? (otherImages = body.otherImages) : (otherImages = []);
  if (body.discount == "null" || body.discount == "") {
    body.discount = 0;
  }
